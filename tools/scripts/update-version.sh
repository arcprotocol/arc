...
if check_file "$PROJECT_ROOT/spec/versions/v1.0/arc-openapi.yaml"; then
  ...
  if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    sed -i '' "s/^  version: \"[0-9]\+\.[0-9]\+\.[0-9]\+\"/  version: \"$NEW_VERSION\"/" "$PROJECT_ROOT/spec/versions/v1.0/arc-openapi.yaml"
  else
    # Linux version
    sed -i "s/^  version: \"[0-9]\+\.[0-9]\+\.[0-9]\+\"/  version: \"$NEW_VERSION\"/" "$PROJECT_ROOT/spec/versions/v1.0/arc-openapi.yaml"
  fi
...
if check_file "$PROJECT_ROOT/spec/versions/v1.0/arc-openrpc.json"; then
  echo "Updating OpenRPC schema version..."
  # For JSON files, we use node to ensure proper JSON formatting
  node -e "
    const fs = require('fs');
    const path = '$PROJECT_ROOT/spec/versions/v1.0/arc-openrpc.json';
